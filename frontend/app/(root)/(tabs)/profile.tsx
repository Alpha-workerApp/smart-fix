...
const Profile = () => {
  ...
  return (
    <SafeAreaView className="">
      <LinearGradient colors={["#171746", "#0F0F19"]} className="px-4 py-2">
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 32, paddingHorizontal: 7 }}>
          ...
          <View className="mt-8 mb-8">
            <Text className="text-white text-2xl font-nunito-bold mb-3">History</Text>
            
            {/* Pending Item - First Row */}
            {pendingData.length > 0 && (
              <View className="flex-row items-center justify-between py-4 border-b border-gray-700">
                <View className="flex-row items-center gap-4">
                  <Image source={{ uri: pendingData[0].img }} className="w-12 h-12 rounded-full" />
                  <View>
                    <Text className="text-white font-nunito-medium text-lg">{pendingData[0].username}</Text>
                    <Text className="text-white font-nunito-light text-sm">{pendingData[0].workertype}</Text>
                  </View>
                </View>
                <View className="text-right">
                  <Text className="text-white font-nunito-bold">{pendingData[0].paymentstatus}</Text>
                  <Text className="text-white font-nunito-light text-sm">${pendingData[0].wageamount}</Text>
                </View>
              </View>
            )}

            {/* Completed & Cancelled Items - Subsequent Rows */}
            {completedAndCancelledData.map((item) => (
              <View key={item.id} className="flex-row items-center justify-between py-6 border-b border-gray-700">
                <View className="flex-row items-center gap-4">
                  <Image source={{ uri: item.img }} className="w-12 h-12 rounded-full" />
                  <View>
                    <Text className="text-white font-nunito-medium text-lg">{item.username}</Text>
                    <Text className="text-white font-nunito-light text-sm">{item.workertype}</Text>
                  </View>
                </View>
                <View className="text-right">
                  <Text className="text-white font-nunito-bold">{item.paymentstatus}</Text>
                  <Text className="text-white font-nunito-light text-sm">${item.wageamount}</Text>
                </View>
              </View>
            ))}
          </View>
